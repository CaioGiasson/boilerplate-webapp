...
| **Google Identity (OAuth 2.0)**                    | Operador (IdP)                                                     | Global (Google)                                                                    | Login/cadastro: e-mail, nome, foto de perfil e `email_verified` no Authorization Code; App não armazena refresh token Google                                                                     | [ ] DPA / termos Google Cloud + aviso na política quando OAuth estiver em produção                                               |
