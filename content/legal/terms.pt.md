# Termos de Uso

**Versão:** 2026-08-21

## 1. Quem oferece o serviço

O Vitraux é oferecido por **Vitraux / Caio Felipe Giasson**. Estes termos regulam o uso da aplicação, incluindo conta, imagens e demais funcionalidades.

## 2. Conta e elegibilidade

Para usar recursos autenticados, o usuário deve criar uma conta com nickname, e-mail, senha e **data de nascimento**, aceitar estes Termos e a Política de Privacidade, e manter as informações atualizadas. O usuário é responsável pelas atividades realizadas com suas credenciais.

**Elegibilidade:** o Vitraux é destinado **apenas a pessoas com 18 anos ou mais**. Não operamos contas de menores; não há fluxo de consentimento parental. Cadastros com idade inferior a 18 anos são recusados. A data de nascimento é armazenada para auditoria do age gate e **não** é exposta a terceiros nos perfis públicos.

## 3. Conteúdo do usuário (UGC)

O usuário declara ter direitos sobre as imagens e textos que envia, inclusive autorização quando a imagem retrata terceiros. Conteúdos ilícitos, que violem direitos de terceiros ou as regras da plataforma podem ser removidos. O Vitraux não se obriga a monitorar todo o conteúdo, mas pode agir para proteger a segurança do serviço e de outros usuários.

## 4. Uso aceitável e conteúdo proibido

É vedado tentar burlar autenticação ou controles de acesso, sobrecarregar a infraestrutura, explorar falhas de segurança, ou usar o serviço para fins ilegais. O armazenamento e o processamento de arquivos podem ocorrer em provedores de nuvem, inclusive fora do Brasil.

Os criadores do Vitraux **desincentivam o uso da plataforma para pornografia**. Conteúdo proibido será **rigorosamente banido**.

É **expressamente proibido** publicar, armazenar ou compartilhar:

- material de abuso sexual infantil (**CSAM**) ou qualquer conteúdo que explore sexualmente menores;
- imagens íntimas de terceiros sem consentimento;
- conteúdo que viole a lei ou a dignidade humana;
- spam, impersonação e demais usos tóxicos descritos nestes termos.

Havendo denúncia e comprovação, o material poderá ser removido e a conta banida, sem prejuízo de outras medidas e da colaboração com autoridades.

## 5. Canal de denúncia

Usuários autenticados podem **reportar imagens** pelo botão “Reportar” no detalhe da imagem (ou via API autenticada). Denúncias entram em fila humana para revisão. O processo legal específico (incluindo obrigações de notificação a autoridades em casos de CSAM) é tratado operacionalmente fora do código da aplicação; o canal de denúncia existe para viabilizar essa revisão.

## 6. Colaboração com autoridades

A plataforma **compromete-se a colaborar com órgãos e entidades legais** no combate a crimes. Isso inclui, na forma da lei, preservar evidências, atender requisições válidas e reportar ilícitos às autoridades competentes.

## 7. Privacidade

O tratamento de dados pessoais está descrito na Política de Privacidade. O cadastro exige o aceite dessa política na versão vigente.

## 8. Limitação

O serviço é prestado “como está”, sujeito a manutenção, instabilidade e evolução de funcionalidades. Limitações de responsabilidade serão redigidas na versão jurídica final.

## 9. Alterações

Estes termos podem ser atualizados. A versão vigente será identificada pela data/versão publicada nesta página. Novos aceites poderão ser exigidos quando houver mudança relevante.

## 10. Contato

Dúvidas: **Caio Felipe Giasson** — **cainitech@gmail.com**.

---

**Placeholder jurídico:** este texto é rascunho operacional e deve ser revisado por assessoria jurídica antes de produção.
